...
func _ready():
	...
	var weapon_sub_viewport = $Head/Camera/WeaponViewport/SubViewport
	weapon_sub_viewport.size = get_viewport().get_visible_rect().size
...
func _process(_delta):
	health_meter.text = str(health)
	
	weapon_camera.global_transform = camera.global_transform
	weapon_camera.fov = (BASE_FOV + camera.fov) / 2
	weapon_camera.global_transform.origin = _head_bob_with_side(t_bob)/2 + camera.global_transform.origin
...
func change_weapon(index):
	...
	weapon_instance.position = Weapons.weapons[index]["position"]
